...
10. Создать функцию, которая принимает несколько чисел через `*args`, коэффициент `multiplier=2` через `**kwargs`, и возвращает словарь, где ключами будут индексы, а значениями — числа, умноженные на коэффициент из `**kwargs`.
