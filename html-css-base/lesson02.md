...
3. Добавьте три параграфа (`p`). Каждый должен начинаться с определения, затем тире, и после тире объяснение. Например:
