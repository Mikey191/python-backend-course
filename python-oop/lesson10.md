...
В следующем уроке мы рассмотрим метод `__bool__`, который позволяет объекту определять своё поведение в булевом контексте — то есть отвечать на вопрос «является ли этот объект истинным?» в конструкциях `if`, `while` и логических операциях.
