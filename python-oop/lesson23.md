...
В уроке 21 мы разбирали паттерн `Template Method`: базовый класс определяет алгоритм, а дочерние классы переопределяют отдельные шаги. Тогда мы использовали такой подход для «абстрактных» методов:
